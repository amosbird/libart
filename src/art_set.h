 ...
 */
bool art_search(const art_tree* t, const unsigned char* key, int key_len) {
    art_node** child;
    art_node* n = t->root;
...
static bool recursive_insert(
    art_node* n, art_node** ref, const unsigned char* key, int key_len, int depth) {
...
RECURSE_SEARCH:;
    ...
    if (child) {
        return recursive_insert(*child, child, key, key_len, depth + 1);
    }
...
bool art_insert(art_tree* t, const unsigned char* key, int key_len) {
    bool inserted = recursive_insert(t->root, &t->root, key, key_len, 0);
    if (inserted)
        t->size++;
    return inserted;
}
...
class art_set {
    art_set_ns::art_tree root;

public:
    art_set() { art_set_ns::art_tree_init(&root); }
    ~art_set() { art_set_ns::art_tree_destroy(&root); }
    bool emplace(const char* p, size_t len) {
        return art_set_ns::art_insert(&root, reinterpret_cast<const unsigned char*>(p), len);
    }
    bool find(const char* p, size_t len) {
        return art_set_ns::art_search(&root, reinterpret_cast<const unsigned char*>(p), len);
    }
